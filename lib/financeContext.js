...
export function FinanceContextProvider({ children }) {
  ...
  // Deletes an expense from a specific Category in Firebase
  const deleteExpenseItem = async (expCategoryId, updatedExpense) => {
    try {
      // Calculate the new total by subtracting the amount of the deleted item
      const newTotal = updatedExpense.items.reduce(
        (sum, item) => sum + item.amount,
        0
      );

      await updateItem(expCategoryId, {
      ...
      setExpenses((prevExpenses) => {
        ...
        if (pos !== -1) {
          ...
          updatedExpenses[pos].total = newTotal;
        }
